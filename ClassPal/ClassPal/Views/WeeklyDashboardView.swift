...
// MARK: - ClassPal Theme & Course Color Palette
public struct ClassPalTheme {
...
public struct WeeklyDashboardView: View {
    ...
    // All course weeks sorted — drives the week filter bar and the reading list
    private var sortedCourseWeeks: [Week] {
        let filtered = allWeeks.filter { $0.course != nil }
        return filtered.sorted { $0.weekNumber < $1.weekNumber }
    }

    // Week numbers that actually exist in the DB (from Week objects)
    private var courseWeekNumbers: [Int] {
        Array(Set(sortedCourseWeeks.map { $0.weekNumber })).sorted()
    }
    ...
    public var body: some View {
        NavigationStack {
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 16) {
                    ...
                    if courses.isEmpty && sortMode != "trash" {
                        ...
                        let activeDisplayNums = displayNums.filter { num in
            ...
            .background(ClassPalTheme.bgCanvas)
    ...
    public var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .padding(.horizontal, 18)
                .padding(.vertical, 10)
                .background(isSelected ? ClassPalTheme.accentBlue : Color.white)
                .foregroundColor(isSelected ? .white : ClassPalTheme.textDark)
                .cornerRadius(22)
                .shadow(color: isSelected ? ClassPalTheme.accentBlue.opacity(0.25) : Color.black.opacity(0.04), radius: isSelected ? 6 : 4, x: 0, y: 2)
        }
        .buttonStyle(.plain)
    }
}
        ...
        HStack(spacing: 10) {
            ...
            HStack(spacing: 12) {
                ...
                Button(action: {
                    ...
                    ZStack {
                        Circle()
                            .fill(reading.isCompleted ? ClassPalTheme.accentBlue : Color.clear)
                            .frame(width: 22, height: 22)
                            .overlay(
                                Circle()
                                    .stroke(reading.isCompleted ? ClassPalTheme.accentBlue : Color(red: 0.75, green: 0.80, blue: 0.86), lineWidth: 1.8)
                            )
