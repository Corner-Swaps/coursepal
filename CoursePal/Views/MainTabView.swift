...
public struct MainTabView: View {
    ...
    public var body: some View {
        ZStack(alignment: .bottom) {
            // Main Tab Content Area (Kept alive in ZStack for zero-latency instant tab switching)
            ZStack {
                WeeklyDashboardView()
                    .opacity(selectedTab == "readings" ? 1 : 0)
                    .allowsHitTesting(selectedTab == "readings")

                AssignmentsView()
                    .opacity(selectedTab == "assignments" ? 1 : 0)
                    .allowsHitTesting(selectedTab == "assignments")
                ...
                SyllabusRepositoryView(showingUploadModal: $showingUploadDocModal, isGlobalProcessing: $isGlobalProcessing, selectedCourseForAddDoc: $selectedCourseForAddDoc)
                    .opacity((selectedTab == "syllabus" || selectedTab == "vault") ? 1 : 0)
                    .allowsHitTesting(selectedTab == "syllabus" || selectedTab == "vault")

                ShareCenterView()
                    .opacity((selectedTab == "share" || selectedTab == "invite") ? 1 : 0)
                    .allowsHitTesting(selectedTab == "share" || selectedTab == "invite")
            }

            // MARK: - Global Floating Upload Progress Banner (with cancel 'X' button)
            if SyllabusUploadManager.shared.isUploading && selectedTab != "syllabus" {
                VStack {
                    HStack(spacing: 10) {
                        ProgressView()
                            .controlSize(.small)
                        ...
                        Text(SyllabusUploadManager.shared.statusText.isEmpty ? "Uploading material..." : SyllabusUploadManager.shared.statusText)
                ...
                ZStack(alignment: .center) {
                    ...
                    HStack(spacing: 0) {
                        ...
                        Button(action: { selectedTab = "readings" }) {
                            ...
                            .foregroundColor(selectedTab == "readings" ? activeThemeColor : Color(red: 0.45, green: 0.52, blue: 0.62))
                        ...
                        Button(action: { selectedTab = "assignments" }) {
                            ...
                            .foregroundColor(selectedTab == "assignments" ? activeThemeColor : Color(red: 0.45, green: 0.52, blue: 0.62))
                            .frame(maxWidth: .infinity)
                            .padding(.vertical, 6)
                        }
                        .buttonStyle(.plain)
                        ...
                        Button(action: { selectedTab = "syllabus" }) {
                            ...
                            .foregroundColor((selectedTab == "syllabus" || selectedTab == "vault") ? activeThemeColor : Color(red: 0.45, green: 0.52, blue: 0.62))
                            .frame(maxWidth: .infinity)
                            .padding(.vertical, 6)
                        }
                        .buttonStyle(.plain)
                    ...
                    Button(action: {
                        ...
                        ZStack {
                            ...
                            Circle()
                                .fill(LinearGradient(colors: [activeThemeColor.opacity(0.85), activeThemeColor], startPoint: .top, endPoint: .bottom))
            ...
            // MARK: - First-Launch Centered Terms & Privacy Agreement Modal
            if !hasAcceptedTerms {
                WelcomeTermsModalView()
    ...
    private func sanitizeDatabase() {
        ...
        for course in courses where !course.isDeleted {
            if course.hexColor.isEmpty || usedColors.contains(course.hexColor.lowercased()) {
                course.hexColor = CourseImporter.getUniqueColor(usedColors: usedColors)
            }
            usedColors.insert(course.hexColor.lowercased())

            for doc in course.syllabusDocs {
                if let hex = doc.docColorHex?.lowercased(), !hex.isEmpty, !usedColors.contains(hex) {
                    usedColors.insert(hex)
                } else {
                    let uniqueDocHex = CourseImporter.getUniqueColor(usedColors: usedColors)
                    doc.docColorHex = uniqueDocHex
                    usedColors.insert(uniqueDocHex.lowercased())
                }
            }
        }
        ...
        for course in courses {
            for week in course.weeks {
                for reading in week.readings {
                    if reading.title.contains("<") || reading.title.contains(">") {
                        let cleanTitle = LocalSyllabusParser.shared.buildStrict5To6WordTitle(from: reading.title)
                        reading.title = cleanTitle.isEmpty ? "Required Reading" : cleanTitle
                    }
                    if let topics = reading.relevantTopics, topics.contains("<") {
                        reading.relevantTopics = DocumentExtractor.sanitizeText(topics)
                    }
                    let rawKey = reading.keyTakeawaysText
                    let cleaned = rawKey
            ...
            for assign in course.assignments {
                if assign.title.contains("<") || assign.title.contains(">") {
                    let cleanTitle = LocalSyllabusParser.shared.buildStrict5To6WordTitle(from: assign.title)
                    assign.title = cleanTitle.isEmpty ? "Assignment" : cleanTitle
                }
                if let topics = assign.relevantTopics, topics.contains("<") {
                    assign.relevantTopics = DocumentExtractor.sanitizeText(topics)
                }
            }
        }
...
public struct UploadDocModalView: View {
    ...
    private func confirmAndProcessFiles(_ urls: [URL]) {
        ...
        var stagedURLs: [URL] = []
        for url in urls {
            stagedURLs.append(PersistentFileStager.stage(url: url))
        }

        onClose?()
        dismiss()
        SyllabusUploadManager.shared.startUpload(urls: stagedURLs, targetCourse: targetCourse, modelContext: modelContext)
    }
    ...
    private func parseAndImportVaultDoc(_ doc: VaultDocument) {
        ...
        if let data = doc.rawFileData, !data.isEmpty {
            let stagedURL = saveTempFile(data: data, filename: doc.title)
            SyllabusUploadManager.shared.startUpload(urls: [stagedURL], targetCourse: targetCourse, modelContext: modelContext)
        } else if let textToParse = doc.fileContent, !textToParse.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            let stagedURL = saveTempFile(data: textToParse.data(using: .utf8) ?? Data(), filename: "\(doc.title).txt")
            SyllabusUploadManager.shared.startUpload(urls: [stagedURL], targetCourse: targetCourse, modelContext: modelContext)
        }
    }

    private func saveTempFile(data: Data, filename: String) -> URL {
        return PersistentFileStager.stage(data: data, filename: filename)
    }
}
...
public struct AddCourseModalView: View {
    ...
    private var canSave: Bool {
        return hasSyllabusSource || !courseName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    }
    ...
    public var body: some View {
        NavigationStack {
            ScrollViewReader { scrollProxy in
                ...
                Section {
                    VStack(spacing: 8) {
                        // Pill 1: Single Course Upload & Background Processing Notice
                        HStack(alignment: .top, spacing: 10) {
                            Image(systemName: "doc.text.viewfinder")
                                ...
                                .padding(.top, 2)
                            VStack(alignment: .leading, spacing: 2) {
                                Text("Upload One Course at a Time")
                                    .font(.system(size: 12, weight: .bold, design: .rounded))
                                    .foregroundColor(Color(red: 0.08, green: 0.12, blue: 0.22))
                                Text("Document processing takes 1–2 minutes. You can safely exit or minimize the app while it runs in the background.")
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 12)
                                .stroke(Color(red: 0.14, green: 0.44, blue: 0.96).opacity(0.20), lineWidth: 1)
                        )
                        ...
                        HStack(alignment: .top, spacing: 10) {
                            Image(systemName: "checkmark.shield.fill")
                                .font(.system(size: 14, weight: .bold))
                                .foregroundColor(Color(red: 0.88, green: 0.45, blue: 0.08))
                                .padding(.top, 2)
                            VStack(alignment: .leading, spacing: 2) {
                                Text("Double-Check Your Syllabus")
                                    .font(.system(size: 12, weight: .bold, design: .rounded))
                                    .foregroundColor(Color(red: 0.08, green: 0.12, blue: 0.22))
                                Text("Please check your original PDF to verify all dates, readings, and assignments imported accurately.")
                                    .font(.system(size: 11, weight: .medium))
                                    .foregroundColor(Color(red: 0.35, green: 0.42, blue: 0.52))
                            }
                        }
                        .padding(10)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .background(Color(red: 0.88, green: 0.45, blue: 0.08).opacity(0.08))
                        .cornerRadius(12)
                        .overlay(
                            RoundedRectangle(cornerRadius: 12)
                                .stroke(Color(red: 0.88, green: 0.45, blue: 0.08).opacity(0.20), lineWidth: 1)
                        )
                    }
                    .padding(.vertical, 2)
                }
                ...
                Section(header: HStack {
                    ...
                    if isProcessingCourse {
                        ...
                        Text("ATTACHED")
                            ...
                            .cornerRadius(6)
                    } else if showValidationHighlight && !hasSyllabusSource {
                        Text("REQUIRED")
                            .font(.system(size: 9, weight: .bold))
                            .foregroundColor(.white)
                            .padding(.horizontal, 6)
                    ...
                    HStack(spacing: 12) {
                        Button(action: {
                            ...
                            HStack(spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    Text(attachedFileName != nil ? "\(attachedFileName!) Attached" : "Upload Class Material")
                                        .font(.system(size: 13, weight: .bold))
                                        .foregroundColor(Color(red: 0.08, green: 0.12, blue: 0.22))
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            if !uploadedFilesList.isEmpty {
                                HStack(spacing: 10) {
                                    ...
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text("\(uploadedFilesList.count) Class Materials Attached")
                                            .font(.system(size: 13, weight: .bold))
                                            .foregroundColor(Color(red: 0.08, green: 0.12, blue: 0.22))
    ...
    private func saveCourse() {
        ...
        var name = courseName.trimmingCharacters(in: .whitespaces)

        if name.isEmpty, let fName = attachedFileName, !fName.isEmpty {
            name = fName.replacingOccurrences(of: #"\.[^.]+$"#, with: "", options: .regularExpression).capitalized
        }
        ...
        // Immediately dismiss modal sheet so there is ZERO UI freeze or visual glitch!
        onCourseCreated?()
        dismiss()
